...
class IntradayShortScreenerPage {
			...
			<div class="tj-is-wrap">
				<div class="tj-is-hero">
					...
					<div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
						...
						<button class="btn-scan" id="tj-is-run-all" style="background:#10b981;color:#fff;">Run All 9 Scans</button>
	...
	_bind_controls() {
		...
		$b.find("#tj-is-universe").on("change", (e) => {
	...
	_start_scan() {
		...
		frappe.call({
			...
			args: { scan_type: "Intraday Short", universe: this.universe },
			callback: (r) => {
				const m = r.message || {};
				if (!m.ok) {
			...
			frappe.call({
				...
				callback: (r) => {
					...
					if (["Queued", "Running"].includes(m.status)) {
						...
						$b.find("#tj-is-run-all").prop("disabled", false).text("Run All 9 Scans");
					}
				},
			});
		};
